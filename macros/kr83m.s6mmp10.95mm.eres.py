...
plt.hist(fast_data.charge, bins=50, label=f'R < 300, SiPM Charge > 42')
...
plt.hist(fast_data.X, bins=50, label=f'R < 300, SiPM Charge > 42')
plt.title(mc['name']+', '+event_type)
plt.xlabel('X [mm]')
plt.savefig(outdir+f'x_r{rcut}_sthresh{sipm_thresh}_'+mc_name+'.png')
plt.close() 

plt.hist(fast_data.Y, bins=50, label=f'R < 300, SiPM Charge > 42')
plt.title(mc['name']+', '+event_type)
plt.xlabel('Y [mm]')
plt.savefig(outdir+f'y_r{rcut}_sthresh{sipm_thresh}_'+mc_name+'.png')
plt.close()

plt.hist(fast_data.r, bins=50, label=f'R < 300, SiPM Charge > 42')
plt.title(mc['name']+', '+event_type)
plt.xlabel('R [mm]')
