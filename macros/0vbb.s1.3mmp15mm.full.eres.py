...
data_dir = '/n/holystore01/LABS/guenette_lab/Lab/data/NEXT/FLEX/mc/fast_v_full_test/nexus/kr83/buffy/reduced_hdf5/'
fast_sims = [data_dir + f'{event_type}.'+mc_file_name+'.' + str(i) + '.buffy.waveforms.h5' for i in range(0, num_files)]
...
plt.hist(fast_data.charge, bins=50, label=f'R < 300, SiPM Charge > 5')
...
plt.hist(fast_data.X, bins=50, label=f'R < 300, SiPM Charge > 5')
plt.title(mc['name']+', '+event_type)
plt.xlabel('X [mm]')
plt.savefig(outdir+f'x_r{rcut}_sthresh{sipm_thresh}_'+mc_name+'.png')
plt.close() 

plt.hist(fast_data.Y, bins=50, label=f'R < 300, SiPM Charge > 5')
plt.title(mc['name']+', '+event_type)
plt.xlabel('Y [mm]')
plt.savefig(outdir+f'y_r{rcut}_sthresh{sipm_thresh}_'+mc_name+'.png')
plt.close()

plt.hist(fast_data.r, bins=50, label=f'R < 300, SiPM Charge > 5')
plt.title(mc['name']+', '+event_type)
plt.xlabel('R [mm]')
...
json.dump(mc, open('data_eres_0vbb_rcut300_sthresh5/'+mc_name+'.txt', 'w'))
