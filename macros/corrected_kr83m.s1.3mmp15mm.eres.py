...
if event_type == 'kr83m':
    ...
    corrections = fast_data.charge * total_correction( fast_data.X, fast_data.Y, fast_data.Z, 0)
    corrected_fast_data = fast_data.copy()
    corrected_fast_data.charge = corrections
    
    # Compare corrections
    plt.hist(fast_data.charge, bins=bins_fit, label='uncorrected')
    plt.hist(corrected_fast_data.charge, bins=bins_fit, label='corrected')
    plt.label('Total SiPM Charge [pes]')
