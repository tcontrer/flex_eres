...
mcs = {'s1.3mmp3.5mm':
        {'size':1.3, 'pitch':3.5, 'teflon':False, 'nsipms':61053,  'run':True, 'dbname':'s13mmp35mm'},
    's1.3mmp2.4mm':
        {'size':1.3, 'pitch':2.4, 'teflon':False, 'nsipms':129889, 'run':True, 'dbname':'s13mmp24mm'},
    's3mmp5.5mm':
        {'size':3,   'pitch':5.5, 'teflon':False, 'nsipms':24748,  'run':True, 'dbname':'s3mmp55mm'},
    's6mmp11mm':
        {'size':6,   'pitch':11,  'teflon':False, 'nsipms':6181,   'run':True, 'dbname':'s6mmp11mm'},
    's6mmp7.8mm':
        {'size':6,   'pitch':7.8, 'teflon':False, 'nsipms':12304,   'run':True, 'dbname':'s6mmp78mm'}}
...
def GetEvents(inputfile):
    ...
    i = 0
    events = []
    wfs = cp.wf_from_files([inputfile], cp.WfType.mcrd)
    try:
        while wfs:
            thisdata = next(wfs)
...
def SiPM_Ds_bary(all_waveforms, sipm_xys):
    """
    Calcualted distance from event center by
    calculating barycenter.
    """
    s2_corona = xya.corona(sipm_xys, np.sum(all_waveforms, axis=1), dbsipm,
                           Qthr            =  0 * units.pes,
                           Qlm             =  1 * units.pes,
    ...
    dx2 = (s2_corona.XY[0] - sipm_xys[:,0])**2
    dy2 = (s2_corona.XY[1] - sipm_xys[:,1])**2
    sipm_ds = np.sqrt(dx2 + dy2)

    return sipm_ds


def GetDistr_bls(dcuts, all_event_waveforms, event_info, raw_events, kdst_events, offset=0, dbname='s13mmp24mm'):
    """
    ...
    dbsipm = DataSiPM(dbname, run_number)
    ...
    for event in range(len(raw_events)):
        ...
        for dcut in dcuts:
            ...
            sipm_ds = SiPM_Ds_bary(all_event_waveforms[raw_evt], sipm_xys)
...
def GetS2andOuter(dcuts, events, event_info, raw_events, kdst_events):
    ...
    calibrated_events = events #calibrate_sipms(events, adc_to_pes, -999, noise_mean)
