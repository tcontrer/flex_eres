...
eresfile_name = 'data_eres_'+event_type+f'_rcut{rcut}_sthresh{sipm_thresh}'
    ...
    for name in mcs:
        ...
        if mc['run']:
            ...
            with open('macros/'+pyfile_name, 'w') as f:
                ...
                f.write("eresfile_name = '/n/holystore01/LABS/guenette_lab/Lab/data/NEXT/FLEX/mc/eres_22072022/eres_taylor/out/"+f"{eresfile_name}'\n")
                f.write("if teflon:\n") 
                ...
                f.write("    fit_range_sipms = (peak - np.std(fast_data.charge)/10., peak + np.std(fast_data.charge)/10.)\n")
                f.write("\n")
                ...
                f.write("\n")
                f.write("json.dump(mc, open(eresfile_name+mc_name+'.txt', 'w'))\n")
